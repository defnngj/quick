...
class RunTest(unittest.TestCase):
    ...
    def test_case(self, url, method, header, params_type, params_body, assert_type, assert_text):
        ...
        if method == MethodType.get:
            ret_text = requests.get(url, headers=header, params=params_body).text
        ...
        if method == MethodType.post:
            if params_type == ParamsType.data:
                ret_text = requests.post(url, headers=header, data=params_body).text
                if assert_type == AssertType.include:
                    self.assertIn(assert_text, ret_text)
                elif assert_type == AssertType.equal:
                    self.assertEqual(assert_text, ret_text)
            ...
            if params_type == ParamsType.json:
                ret_text = requests.post(url, headers=header, json=params_body).text
                if assert_type == AssertType.include:
                    self.assertIn(assert_text, ret_text)
                elif assert_type == AssertType.equal:
                    self.assertEqual(assert_text, ret_text)
        ...
        if method == MethodType.put:
            if params_type == ParamsType.data:
                ret_text = requests.put(url, headers=header, data=params_body).text
                if assert_type == AssertType.include:
                    self.assertIn(assert_text, ret_text)
                elif assert_type == AssertType.equal:
                    self.assertEqual(assert_text, ret_text)
            ...
            if params_type == ParamsType.json:
                ret_text = requests.put(url, headers=header, json=params_body).text
                if assert_type == AssertType.include:
                    self.assertIn(assert_text, ret_text)
                elif assert_type == AssertType.equal:
                    self.assertEqual(assert_text, ret_text)
        ...
        if method == MethodType.delete:
            if params_type == ParamsType.data:
                ret_text = requests.delete(url, headers=header, data=params_body).text
                if assert_type == AssertType.include:
                    self.assertIn(assert_text, ret_text)
                elif assert_type == AssertType.equal:
                    self.assertEqual(assert_text, ret_text)
                    ...
                    pass
            if params_type == ParamsType.json:
                ret_text = requests.delete(url, headers=header, json=params_body).text
                if assert_type == AssertType.include:
                    self.assertIn(assert_text, ret_text)
                elif assert_type == AssertType.equal:
                    self.assertEqual(assert_text, ret_text)
